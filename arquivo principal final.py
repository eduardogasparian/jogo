...
class Game:
    ...
    def load_data(self):
        ...
        img_folder = path.join('PNG')
        snd_folder = path.join('snd')
        music_folder = path.join( 'music')
        map_folder = path.join('tiled maps')
        self.hud_font = path.join('ZOMBIE.TTF')
        self.title_font = path.join('ZOMBIE.TTF')
        self.dim_screen = pg.Surface(self.screen.get_size()).convert_alpha()
        self.dim_screen.fill((0, 0, 0, 180))
        self.map = TiledMap(path.join('novo mapa.tmx'))
        self.map_img = self.map.make_map()
        self.map.rect = self.map_img.get_rect()
        self.player_img = pg.image.load(path.join('Hitman 1', PLAYER_IMG)).convert_alpha()
        self.bullet_img = pg.image.load(path.join(BULLET_IMG)).convert_alpha()
        self.mob_img = pg.image.load(path.join('Zombie 1',MOB_IMG)).convert_alpha()
        self.monstro_img=pg.image.load(path.join(MONSTRO_IMG)).convert_alpha()
    ...
    def run(self):
        ...
        self.playing = True
        pygame.mixer.music.load('music/Musica Zombies.mp3')
    ...
    def show_go_screen(self):
        self.screen.fill(BLACK)
        self.draw_text("ACABOU", self.title_font, 100, RED,
                       WIDTH / 2, HEIGHT / 2, align="center")
        self.draw_text("Press a key to start", self.title_font, 75, WHITE,
                       WIDTH / 2, HEIGHT * 3 / 4, align="center")
        pg.display.flip()
